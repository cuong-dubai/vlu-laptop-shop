...
switch ($act) {
    ...
    case "man_member":
        viewMembers();
        $template = "user/man_user/mans";
        break;
    case "add_member":
        $template = "user/man_user/man_add";
        break;
    case "edit_member":
        editMember();
        $template = "user/man_user/man_add";
...
function viewMembers()
{
    ...
    $sql = "select * from #_customers where id <> 0 $where order by numb,id desc $limit";
    $items = $d->rawQuery($sql);
    $sqlNum = "select count(*) as 'num' from #_customers where id <> 0 $where order by numb,id desc";
...
function editMember()
{
    ...
    if (empty($id)) {
        ...
        $item = $d->rawQueryOne("select * from #_customers where id = ? limit 0,1", array($id));
...
function saveMember()
{
    ...
    if (!empty($data['username'])) {
        if ($func->checkAccount($data['username'], 'username', 'customers', $id)) {
            $response['messages'][] = 'Tài khoản đã tồn tại';
        }
    }
    ...
    if (!empty($data['email'])) {
        if ($func->checkAccount($data['email'], 'email', 'customers', $id)) {
            $response['messages'][] = 'Email đã tồn tại';
        }
    }
...
function deleteMember()
{
    ...
    if ($id) {
        $row = $d->rawQueryOne("select id from #_customers where id = ? limit 0,1", array($id));

        if (!empty($row['id'])) {
            $d->rawQuery("delete from #_customers where id = ?", array($id));
